...
class SQLiteWriter(ds: DataSource, fs: FunctionalitySettings) extends SnapshotStateReader with StateWriter with History {
  ...
  override def featureVotes(height: Int) = readOnly { implicit s =>
    ...
    val windowStart = height / windowSize * windowSize + 1
  ...
  private def storeWavesBalances(diff: Diff)(implicit session: DBSession) = {
  ...
  private def storeAssetBalances(diff: Diff)(implicit session: DBSession) = {
